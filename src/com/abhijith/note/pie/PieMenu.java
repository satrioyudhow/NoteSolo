...
public class PieMenu extends FrameLayout {
	...
	private static final long ANIMATION = 50;
	...
	private void init(Context context) {
		...
		mBackground = res.getDrawable(R.drawable.pie_background_normal);
		mNormalPaint = new Paint();
		mNormalPaint.setColor(res.getColor(R.color.pie_normal));
		mNormalPaint.setAntiAlias(true);
		mSelectedPaint = new Paint();
		mSelectedPaint.setColor(res.getColor(R.color.pie_selected));
		mSelectedPaint.setAntiAlias(true);
		mSubPaint = new Paint();
		mSubPaint.setColor(res.getColor(R.color.pie_sub));
		mSubPaint.setAntiAlias(true);
	}

	public void setController(PieController controller) {
	...
	private void layoutPie() {
		...
		for (int i = 0; i < mLevels; i++) {
			...
			mPath = makeSlice(getDegrees(0) - gap, getDegrees(sweep) + gap,
					outer, inner, mCenter);
			for (PieItem item : mCurrentItems) {
	 ...
	 * @return skia angle
	 */
	private float getDegrees(float angle) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (MotionEvent.ACTION_DOWN == action) {
			...
			if (mOpen) {
				...
				if (mPieView != null) {
					handled = mPieView.onTouchEvent(event);
				}
			...
			return false;
		} else if (MotionEvent.ACTION_MOVE == action && getParent()!=null) {
			...
			if (polar.y > maxr) {
				...
				if (getParent() != null) {
					((ViewGroup) getParent()).dispatchTouchEvent(event);
				}
				return false;
			}
			...
			if (item == null) {
				...
				if ((item != null) && item.isPieView()
						&& (item.getView() != null)) {
					int cx = item.getView().getLeft()
							+ (onTheLeft() ? item.getView().getWidth() : 0);
	...
	private void animateIn(final PieItem fixed, AnimatorListener listener) {
		...
		anim.addUpdateListener(new AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				for (PieItem item : mCurrentItems) {
					if (item != fixed) {
						item.setAnimationAngle((1 - animation
								.getAnimatedFraction())
								* (target - item.getStart()));
					}
				}
